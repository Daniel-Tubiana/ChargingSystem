...
class EVChargingEnv_discrete(gym.Env):
    def __init__(self, evs, power_limit):
        super(EVChargingEnv_discrete, self).__init__()
    ...
    def step(self, actions):
        ...
        # fill charge vector in power values
        ...
        # Update the state of each EV in self.evs
        for idx, ev in enumerate(self.evs):
            ...
            if (ev['Arrival_time[h]'] < self.time) | (ev['TuD (int)'] <= 0):
                if charge_actions[actions[idx]] > 0:
                    charge_time_reward -= 100000
                else:
                    #charge_time_reward += 100  # TODO: set reward to zero
                    charge_time_reward += 0
                # pass
        ...
        """ 2. Power constraint"""
        if np.sum(charge) > self.power_limit[self.time]:
            power_reward = -100000

        elif np.sum(charge) < self.power_limit[self.time]:
            #power_reward = 100  # TODO: set reward to zero
            power_reward += 0
        ...
        if self.time == 24:
            ...
            for idx, ev in enumerate(self.evs):
                ...
                min_SOC = 0.2
                if ev['SOC'] >= ENonD_SOC + min_SOC:
                    SOC_reward += 0
                elif ev['SOC'] < ENonD_SOC + min_SOC:
                    SOC_reward -= 5000  # negative reward -> SOC didn't reach to req level
...
def make_ev_charging_env(evs, power_limit):
    return EVChargingEnv_discrete(evs=evs, power_limit=power_limit)
